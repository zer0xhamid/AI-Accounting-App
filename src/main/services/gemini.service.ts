...
10ز. "رأس المال" أو "رصيد افتتاحي" أو "تعديل رصيد" أو "حطيت في الخزنة" أو "سحبت من الخزنة" أو "رصيد أول المدة" أو "ضيف على الرصيد" أو "عدّل الرصيد" أو "زوّد الرصيد" أو "نقّص الرصيد" = type يساوي "تعديل_رصيد"
...
13أ. لو ذكر سعر الوحدة أو سعر كل صنف، حطه في unit_price. لو مذكرش → unit_price = 0
...
الرد:
{"type":"شراء","person":"أحمد","date":null,"items":[{"name":"خشب زان","quantity":10,"unit":"لوح","specs":null,"unit_price":500}],"total_amount":5000,"paid_amount":3000,"remaining_amount":2000,"payment_method":"كاش","notes":null}

المستخدم: "بعت لخالد مطبخ بـ 80 ألف آجل"
الرد:
{"type":"بيع","person":"خالد","date":null,"items":[{"name":"مطبخ","quantity":1,"unit":"قطعة","specs":null,"unit_price":80000}],"total_amount":80000,"paid_amount":0,"remaining_amount":80000,"payment_method":"آجل","notes":null}

المستخدم: "بعت 15 باب لمحمد أبو عمر بـ 45 ألف ودفع 20 ألف"
الرد:
{"type":"بيع","person":"محمد أبو عمر","date":null,"items":[{"name":"باب","quantity":15,"unit":"قطعة","specs":null,"unit_price":3000}],"total_amount":45000,"paid_amount":20000,"remaining_amount":25000,"payment_method":"كاش","notes":null}
...
الرد:
{"type":"مصروف","person":null,"date":null,"items":[{"name":"إيجار المحل","quantity":1,"unit":"عدد","specs":null,"unit_price":5000}],"total_amount":5000,"paid_amount":5000,"remaining_amount":0,"payment_method":"كاش","expense_category":"إيجار","notes":null}

المستخدم: "اشتريت 20 لوح موسكي 2.4 متر و 15 لوح زان 3 متر بـ 45 ألف من محمد ودفعت النص"
الرد:
{"type":"شراء","person":"محمد","date":null,"items":[{"name":"خشب موسكي","quantity":20,"unit":"لوح","specs":"2.4 متر","unit_price":0},{"name":"خشب زان","quantity":15,"unit":"لوح","specs":"3 متر","unit_price":0}],"total_amount":45000,"paid_amount":22500,"remaining_amount":22500,"payment_method":"كاش","notes":null}

المستخدم: "بعت لشركة المعمار 3 غرف نوم و 2 دولاب بـ 200 ألف ودفعوا 100 ألف"
الرد:
{"type":"بيع","person":"شركة المعمار","date":null,"items":[{"name":"غرفة نوم","quantity":3,"unit":"قطعة","specs":null,"unit_price":0},{"name":"دولاب","quantity":2,"unit":"قطعة","specs":null,"unit_price":0}],"total_amount":200000,"paid_amount":100000,"remaining_amount":100000,"payment_method":"كاش","notes":null}

المستخدم: "بعت 5 ابواب كل باب 3 متر طول و عرض نص متر حشو MDF لخالد بـ 25 ألف كاش"
الرد:
{"type":"بيع","person":"خالد","date":null,"items":[{"name":"باب","quantity":5,"unit":"قطعة","specs":"طول 3 متر × عرض 0.5 متر حشو MDF","unit_price":5000}],"total_amount":25000,"paid_amount":25000,"remaining_amount":0,"payment_method":"كاش","notes":null}

المستخدم: "بعت مطبخ 6 متر لأحمد بـ 120 ألف ودفع 50 ألف"
الرد:
{"type":"بيع","person":"أحمد","date":null,"items":[{"name":"مطبخ","quantity":1,"unit":"قطعة","specs":"6 متر","unit_price":120000}],"total_amount":120000,"paid_amount":50000,"remaining_amount":70000,"payment_method":"كاش","notes":null}

المستخدم: "ضيف في المخزن 10 لوح خشب موسكي و 5 لوح أبلكاش و 8 لوح MDF"
الرد:
{"type":"إضافة_مخزن","person":null,"date":null,"items":[{"name":"خشب موسكي","quantity":10,"unit":"لوح","specs":null,"unit_price":0},{"name":"أبلكاش","quantity":5,"unit":"لوح","specs":null,"unit_price":0},{"name":"MDF","quantity":8,"unit":"لوح","specs":null,"unit_price":0}],"total_amount":0,"paid_amount":0,"remaining_amount":0,"payment_method":"كاش","notes":"إضافة مخزون"}
...
المستخدم: "رأس المال 500 ألف"
الرد:
{"type":"تعديل_رصيد","person":null,"date":null,"items":[],"total_amount":500000,"paid_amount":500000,"remaining_amount":0,"payment_method":"كاش","notes":"رأس المال"}

المستخدم: "حطيت في الخزنة 20 ألف"
الرد:
{"type":"تعديل_رصيد","person":null,"date":null,"items":[],"total_amount":20000,"paid_amount":20000,"remaining_amount":0,"payment_method":"كاش","notes":"إضافة للخزنة"}

المستخدم: "سحبت من الخزنة 5000"
الرد:
{"type":"تعديل_رصيد","person":null,"date":null,"items":[],"total_amount":-5000,"paid_amount":-5000,"remaining_amount":0,"payment_method":"كاش","notes":"سحب من الخزنة"}`

interface AIParseResult {
  type: string
  person: string | null
  date: string | null
  items: { name: string; quantity: number; unit: string; specs: string | null; unit_price: number }[]
  total_amount: number
  paid_amount: number
...
export class GeminiService {
  ...
  private autoBomMatch(results: AIParseResult[]): void {
    const templates = this.db.prepare('SELECT * FROM bom_templates ORDER BY name').all() as Array<{ id: number; name: string; notes: string | null }>
    if (templates.length === 0) return

    const normalize = (s: string) => s.replace(/[\s_\-ـ]/g, '').replace(/[أإآ]/g, 'ا').replace(/[ة]/g, 'ه').replace(/[ى]/g, 'ي').toLowerCase()

    for (const r of results) {
      if (r.type !== 'بيع' || r.items.length === 0) continue

      const selections: { template_id: number; quantity: number }[] = []

      for (const item of r.items) {
        const itemNorm = normalize(item.name)
        const itemSpecs = item.specs ? normalize(item.specs) : ''
        const combined = itemNorm + ' ' + itemSpecs

        let bestMatch: typeof templates[0] | null = null

        for (const tmpl of templates) {
          const tmplName = normalize(tmpl.name)

          if (tmplName === itemNorm) {
            bestMatch = tmpl
            break
          }
          if (combined.includes(tmplName) || tmplName.includes(itemNorm)) {
            bestMatch = tmpl
          }
        }

        if (bestMatch) {
          const existing = selections.find(s => s.template_id === bestMatch!.id)
          if (existing) {
            existing.quantity += item.quantity
          } else {
            selections.push({ template_id: bestMatch.id, quantity: item.quantity })
          }
        }
      }

      if (selections.length > 0) {
        r.bom_selections = selections
      }
    }
  }

  private autoCategoryMatch(results: AIParseResult[]): void {
    const categories = this.db.prepare('SELECT * FROM inventory_categories ORDER BY name').all() as Array<{ id: number; name: string }>
    if (categories.length === 0) return

    const normalize = (s: string) => s.replace(/[\s_\-ـ]/g, '').replace(/[أإآ]/g, 'ا').replace(/[ة]/g, 'ه').replace(/[ى]/g, 'ي').toLowerCase()

    for (const r of results) {
      if (r.type !== 'شراء' && r.type !== 'إضافة_مخزن') continue
      ...
      for (const item of r.items) {
        ...
        for (const cat of categories) {
          ...
          if (combined.includes(catNorm) || catNorm.includes(itemNorm)) {
            (item as Record<string, unknown>).category_id = cat.id
            break
          }
        }
      }
  ...
  async parseText(text: string): Promise<{ success: boolean; data?: AIParseResult[]; error?: string; usage?: { count: number; limit: number } }> {
    ...
    try {
      ...
      this.autoBomMatch(results)
      this.autoCategoryMatch(results)
  ...
  async chat(question: string): Promise<{ success: boolean; answer?: string; error?: string }> {
    ...
    const personsData = this.db.prepare(`
      ...
      FROM persons p ORDER BY ABS(balance) DESC LIMIT 50
